...
# read in the child behavior checklist dataset
mh_p_cbcl = read.csv(paste0(path, "abcd-data-release-5.1/core/mental-health/mh_p_cbcl.csv"))

# read in the ADHD medication dataset created for a previous project
adhd_meds = read.csv(paste0(path, "adhd_medication/adhd_meds.csv"))
```
...
discNew = discNew %>%
  ...
  mutate(
    ...
    # recode the yes/no discrimination items into dummy variables (values of "don't know" and "refused to answer" are set to missing)
    across(c(dim_yesno_q1, dim_yesno_q2), ~ case_when(.x == 0 ~ 0,
                                                      .x == 1 ~ 1)))

# create dichotomous versions of the "measure of perceived discrimination" items such that 0 = almost never; 1 = rarely, sometimes, often, or very often (values of "don't know" and "refused to answer" are set to missing)
for(i in 1:7) {
  discNew = discNew %>%
    mutate(!!sym(paste0("dim_matrix_q", i, "_b")) := case_when(!!sym(paste0("dim_matrix_q", i)) == 1 ~ 0,
                                                               !!sym(paste0("dim_matrix_q", i)) %in% 2:5 ~ 1))
}; rm(i)

# check the cronbach's alpha for the 9 binary discrimination items
psych::alpha(discNew %>% select(dim_yesno_q1, dim_yesno_q2, dim_matrix_q1_b, dim_matrix_q2_b, dim_matrix_q3_b, dim_matrix_q4_b, dim_matrix_q5_b, dim_matrix_q6_b, dim_matrix_q7_b))[["total"]][["raw_alpha"]]

discNew = discNew %>%
  mutate(
    # calculate the sum of the "yes" / "1" responses for the nine discrimination items
    disc_num = rowSums(select(., c(dim_yesno_q1, dim_yesno_q2, dim_matrix_q1_b, dim_matrix_q2_b, dim_matrix_q3_b, dim_matrix_q4_b, dim_matrix_q5_b, dim_matrix_q6_b, dim_matrix_q7_b)), na.rm = TRUE),
    
    # calculate how many non-missing responses there are for the nine discrimination items
    disc_den = rowSums(!is.na(select(., c(dim_yesno_q1, dim_yesno_q2, dim_matrix_q1_b, dim_matrix_q2_b, dim_matrix_q3_b, dim_matrix_q4_b, dim_matrix_q5_b, dim_matrix_q6_b, dim_matrix_q7_b)))))

# select just the variables of interest
discNew = discNew %>% select(src_subject_id, eventnum, dim_yesno_q1, dim_yesno_q2, dim_y_ss_mean, disc_num, disc_den)

# reshape the dataset to a wide format on time
discNew = discNew %>% pivot_wider(names_from = eventnum, values_from = c(dim_yesno_q1, dim_yesno_q2, dim_y_ss_mean, disc_num, disc_den))
```
...
```{r cbcl, echo = TRUE, message = FALSE}
cbclNew = mh_p_cbcl %>%
  # create a simpler numeric version of the eventname
...
cbclNew = cbclNew %>%
  # select just just the identifying information and t-scores
  select(src_subject_id, eventnum, matches("_t$")) %>%
...
```{r adhd_meds, echo = TRUE, message = FALSE}
adhdNew = adhd_meds %>%
  # convert the boolean variables for stimulant and non-stimulant adhd medication use to a 0/1 format
  mutate(across(c(adhdStim, adhdNonStim), as.numeric))
...
adhdNew = adhdNew %>%
  ...
  # reshape the dataset to a wide format on time
  pivot_wider(names_from = eventnum, values_from = c(adhdStim, adhdNonStim))
```
...
abcd_master = list(longNew, demoNew, discNew, subAll, subIntent, gish, cbclNew, adhdNew) %>% reduce(full_join, by = c("src_subject_id"))

# save the file locally
save(abcd_master, file = paste0(path, "master/abcd_master.RDa"))
```